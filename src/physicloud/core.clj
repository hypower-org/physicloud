...
(defn ^{:private true} genchan
  ...
  (if (contains? (merge @(:external-channel-list unit) @(:internal-channel-list unit)) data)
    (do    
    ...
    ;else - Looks like the CPU needs to get the data over the network!
    (let [net-data (let [
       ...
       (if-not (empty? net-data)
        ...
        (do
          ...
          (if lock 
            (unlock unit))           
          nil))
       (println "the "data" data was in the local list!.......return nil!!!!")nil))))
