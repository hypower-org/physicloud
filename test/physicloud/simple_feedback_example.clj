...
(def simple-feedback-instance
  (let [reference (w/vertex :ref [] ; Declared the reference input waterway. It has no dependencies.
                            (fn []
                              (s/periodically 1000 (fn [] 1)))) ; Generate a reference input of 1 every 500 ms.
      
       plant (w/vertex :plant-output [:plant-output :ref] ; Make a plant and have it require its own information and the reference.
                        (fn 
       ...
       printer (w/vertex :printer [:plant-output] ; Just a sink that prints the result of the feedback loop.
