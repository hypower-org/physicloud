...
(defn del-objective-function
  ...
  (let [k (:k agent)
        pmax (:max agent)]
    (* 2 (* (- 1 k) pmax ((:state agent) (:id agent))) (* (- 1 k) pmax))))
...
(defn global-constraint
  ...
  (let [states (map (fn [x] (+ (* (:k x) (:max x)) (* (- 1 (:k x)) (:max x) ((:state x) (:id x))))) agents)] 
      
      (concat [(- (reduce + states) (reduce + (map :tar agents)))]
              
              (mapv (fn [x] (positive (- (+ (:k x) (* (- 1 (:k x)) ((:state x) (:id x)))) 1))) agents))))

(defn del-global-constraint 
  [agent]
    
  (concat [(- 1 (:k agent))] (mapv (fn [x] (if (= x (:id agent)) (- 1 (:k agent)) 0)) (range (count (:state agent))))))
...
(defn aggregator-fn 
  ...
  (conj states (mapv (fn [x] (+ (* (:k x) (:max x)) (* (- 1 (:k x)) (:max x) ((:state x) (:id x))))) agents)))
...
(def agents 
  ...
  (let [as
        ...
        [{:state [1 1 1 1 1] :control (vec (repeat 6 0)) :id 0 :max 50 :k 0.2 :tar 15}
  
        {:state [1 1 1 1 1] :control (vec (repeat 6 0)) :id 1 :max 5 :k 0.1 :tar 17}
  
        {:state [1 1 1 1 1] :control (vec (repeat 6 0)) :id 2 :max 10 :k 0.15 :tar 5}
  
        {:state [1 1 1 1 1] :control (vec (repeat 6 0)) :id 3 :max 5 :k 0.23 :tar 3}
  
        {:state [1 1 1 1 1] :control (vec (repeat 6 0)) :id 4 :max 15 :k 0.4 :tar 12}]
...
(def outline 
  ...
  {:agent-one 
   ...
   {:tributaries [:aggregator :agent-one :agent-two :agent-three :agent-four :agent-five] 
...
(defn cur-power 
  [agent] 
  (+ (* (:k agent) (:max agent)) (* (- 1 (:k agent)) (:max agent) ((:state agent) (:id agent)))))
